<?php
	...
	require('v_image_errors.inc.php');
	global $filemask;
	$filemask="0777";
	class v_Image
		{
			...
			function v_Image($file_original,$gd2=false,$jpg=70)
				{
					//constructor of the class
					//it takes given file and creates image out of it
					global $ERR;
					$this->clear(); // clear all.
					$file_original=trim($file_original);
					if(file_exists($file_original))
						{
							...
							$this->filename_original=substr($file_original,strrpos($file_original,'/')+1);
							$this->use_gd2=$gd2;
							if ($gd2)
								{
									if (($jpg>0)&&($jpg<101))
										{
											$this->jpeg_quality=$jpg;
											$this->target_format="JPG";}
									else
										{
											$this->error($ERR["JPG_QUALITY_SETTINGS_NOT_CORRECT"]." file=$file_original");
											return false;
										}
								}
							else
		...
		function output_original($destination_file)
			{
				//outputs original image
				//if destination file is empty  image will be output to browser
				// right now $image_type can be JPG or PNG
				return _output_image($destination_file, $this->target_format, $this->image_original);
				}

		function output_resized($path)
			{
				//if destination file is empty  image will be output to browser
				// right now $image_type can be JPG or PNG
				$res = $this->_output_image($path, $this->target_format, $this->image_resized);
		...
		function _output_image($path,$image_type, $image)
			{
				//if destination file is empty  image will be output to browser
				// right now $image_type can be JPEG or PNG
				global $ERR;
				global $filemask;
				$filenameOriginal=trim($this->filename_original);
				$filenameResized=substr($filenameOriginal,0,strrpos($filenameOriginal,'.'));
				$res = false;
				if($image)
					{
						switch($image_type)
							{
								...
								case 'JFIF':
								case 'JPG':
													$res = ImageJpeg($image, $path.$filenameResized.".jpg", $this->jpeg_quality);
													$this->filename_resized=$filenameResized.".jpg";
													chmod(($path.$filenameResized.".jpg"),octdec($filemask));
													break;
								case 'PNG':
													$res = Imagepng($image, $path.$filenameResized.".png");
													$this->filename_resized=$filenameResized.".png";
													chmod(($path.$filenameResized.".png"),octdec($filemask));
													break;
								default:
				...
				if(!$res) $this->error($ERR["UNABLE_TO_OUTPUT"].$path.$filenameResized.".png");
				return $res;
			}
}
